...
当前主文件 `dogbone.lsp` 是 `V2.1-Nest-Compact` 版本。
...
4. 命令行出现 `Dogbone plugin V2.1-Nest-Compact loaded. Commands: DBVER, DBSET, DB1, DBDEBUG, DBAUTO, DBADD, DBRESTORE, DBRESTOREALL, DBNSET, DBNEST, DBNESTM.` 后即可使用。
5. 如需确认 AutoCAD 当前加载的是这份文件，输入 `DBVER`，应显示 `V2.1-Nest-Compact`。
...
- 现有 C 型 dogbone 只自动应用于 sharp line-line 材料内角；不会把 line-arc 接点或外凸榫头角套用到这个凹角算法上。
...
运行时会输出 `DBNEST-DIAG`，包含当前版本、原始选择实体数、分组组件数、板框间距、组件间距、边缘留边、最终选用的排序策略和尾板回填状态。若看不到 `V2.1-Nest-Compact`，说明 AutoCAD 没有加载当前这份 `dogbone.lsp`。
...
3. 输入复制板框之间的水平间距，直接回车使用默认 `50.0`；输入 `0` 会保留默认正间距。
